...
function onMissingConfig()
{
  var ecard = new UI.Card({
    ...
    body: 'Please run the configuration to set the server name and Pokemon filter and restart the app',
    status: statusDef
  });
  ecard.show();
}
...
{
  whitelist = Settings.option("whitelist");
  servername = Settings.option("servername");
  if(!whitelist || !servername)
    {
      onMissingConfig();
    } else {
      buzz_inrange = Settings.option('buzz_inrange');
      buzz_added = Settings.option('buzz_added');
      //var token = Settings.data('authToken');
      //if(token)
      //  {
      //    authToken = token;
      //  }
...
{
  var url = servername + '/next_loc?lat=' + pos.coords.latitude + '&lon=' + pos.coords.longitude;
  console.log(url);
  xhrRequest(url, 'POST', function(res) { }, pos);
}
...
function onLocationSuccess(pos)
{
  ...
  //try hotstart
  //var _pokelist = Settings.data('pokelist');
  //if(_pokelist && Object.keys(_pokelist).length > 0)
//     {
//       hotstart!
//       pokelist = _pokelist;
//       loading = false;
...
var xhrRequest = function (url, type, callback, location) {
  var xhr = new XMLHttpRequest();
  xhr.onload = function () {
    callback(this.responseText, location);
  };
...
function getViewLoc(location)
{
  var diff = 0.009;
  return {
    swLat: location.coords.latitude - diff,
    swLng: location.coords.longitude - diff,
    neLat: location.coords.latitude + diff,
    neLng: location.coords.longitude + diff
  };
}

function getMapData(location)
{
  var pos= getViewLoc(location);
  var url = servername + '/raw_data?pokemon=true&pokestops=false&gyms=false&scanned=false&' + 
      'swLat=' + pos.swLat + '&swLng=' + pos.swLng + '&neLat=' + pos.neLat + '&neLng=' + pos.neLng;
  console.log(url);
  xhrRequest(url, 'GET', onDataFetchComplete, location);
}
...
//   Log('Done fetching data. ' + response);
  ...
  if(!(json && json.pokemons))
    onGetDataError();
  //wait for job
 
   
      processPokelist(json.pokemons);
      if(loading)
        {
          loading = false;
          listUI.show();
...
function processPokelist(items)
{
  ...
  for(var i = 0; i < items.length;i++)
    {
      var pokemon = items[i];
      pokemon.id = pokemon.encounter_id.hashCode();
      if(pokemon.id < 0)
        pokemon.id = pokemon.id * -1;//make positive
      pokemon.disappear_time = Math.round(pokemon.disappear_time / 1000);
      pokemon.id = 'P' + pokemon.id;
//       console.log(whitelist.indexOf(pokemon.pokemon_id));
//       console.log(pokelist);
      if(!pokelist[pokemon.id] && whitelist.indexOf(pokemon.pokemon_id + '') > 1)
        {
          console.log('insert!');
          //insert
          pokelist[pokemon.id] = pokemon;
          vibrate = true;
        }
    }  
...
// {
//   Settings.data('pokelist', pokelist);
//   Log('Saved pokelist: ' + JSON.stringify(pokelist));
// }
...
function processPokemon(location, list)
{
  ...
  for(var key in list)
  {
    var now = Math.floor(Date.now() / 1000);
     if(now > list[key].disappear_time)
        {
          //remove
    ...
    if((pokemon.distance < olddistance && pokemon.distance <= 50 && olddistance > 50) || (pokemon.distance <= 50 && !olddistance))
      {
        ...
        Log(pokemon.pokemon_name + ' in range! Distance ' + pokemon.distance + 'm, old ' + olddistance);
      }
  else { 
    pokemon.inrange = false;
  }
	pokemon.heading = Math.round(mylocation.bearingTo(pokelocation));
    pokemon.remaining = getRemainingTime(pokemon.disappear_time);
  return pokemon;
}
...
function parsePokemonToItems(items)
{
  ...
  for(var i = 0; i < items.length; i++)
    {
      item = {
        title: items[i].pokemon_name + ' (' + items[i].remaining + ')',
